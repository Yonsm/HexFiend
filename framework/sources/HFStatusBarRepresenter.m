...
- (NSString *)describeLength:(unsigned long long)length {
    switch (statusMode) {
        case HFStatusModeDecimal: return [NSString stringWithFormat:NSLocalizedString(@"%llu byte%s", @"%llu 字节"), length, length == 1 ? "" : "s"];
        case HFStatusModeHexadecimal: return [NSString stringWithFormat:NSLocalizedString(@"0x%llX byte%s", @"0x%llX 字节"), length, length == 1 ? "" : "s"];
        case HFStatusModeApproximate: return [NSString stringWithFormat:@"%@", HFDescribeByteCount(length)];
        default: [NSException raise:NSInternalInconsistencyException format:NSLocalizedString(@"Unknown status mode %lu", @"位置状态模式 %lu"), (unsigned long)statusMode]; return @"";
    }
}
...
- (NSString *)stringForEmptySelectionAtOffset:(unsigned long long)offset length:(unsigned long long)length {
    return [NSString stringWithFormat:NSLocalizedString(@"%@ out of %@", @"%@ / %@"), [self describeOffset:offset], [self describeLength:length]];
}

- (NSString *)stringForSingleByteSelectionAtOffset:(unsigned long long)offset length:(unsigned long long)length {
    return [NSString stringWithFormat:NSLocalizedString(@"Byte %@ selected out of %@", @"已选择 %@ / %@"), [self describeOffset:offset], [self describeLength:length]];
}

- (NSString *)stringForSingleRangeSelection:(HFRange)range length:(unsigned long long)length {
    return [NSString stringWithFormat:NSLocalizedString(@"%@ selected at offset %@ out of %@", @"已选择 %@ 于 %@ / %@"), [self describeLength:range.length], [self describeOffsetExcludingApproximate:range.location], [self describeLength:length]];
}
...
- (NSString *)stringForMultipleSelectionsWithLength:(unsigned long long)multipleSelectionLength length:(unsigned long long)length {
    return [NSString stringWithFormat:NSLocalizedString(@"%@ selected at multiple offsets out of %@", @"已选择 %@ 于多处 / %@"), [self describeLength:multipleSelectionLength], [self describeLength:length]];
}
